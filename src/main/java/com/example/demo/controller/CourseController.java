...
public class CourseController {
    ...
    public Result<CourseVO> createCourse(@Valid @RequestBody CourseCreateRequest request) {
        ...
        return Result.success("创建成功", courseVO);
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "更新课程", description = "更新课程信息")
    public Result<CourseVO> updateCourse(@PathVariable Long id, @RequestBody CourseUpdateRequest request) {
    ...
    public Result<List<CourseVO>> getCoursesByTeacherId(@PathVariable Long teacherId) {
